...
type Panel = null|'route'|'style'|'settings'|'layers'
type Page = 'canvas'|'history'
...
const sampleHistory = [
  {id:1,title:'Aug 18 Route',date:'Aug 18, 2026',detail:'1:1 · Steel Blue'},
  {id:2,title:'Weekend Route',date:'Aug 16, 2026',detail:'4:5 · Steel Blue'},
  {id:3,title:'Morning Route',date:'Aug 12, 2026',detail:'3:1 · Steel Blue'},
]
...
export default function App(){
  ...
  const basePoints=useMemo(()=>{
    ...
    // Always fit into a square coordinate system. This prevents route stretching.
  ...
  const routeD=linePath(basePoints,smooth)
  ...
  const onMapTouchStart=(e:React.TouchEvent)=>{
    if((e.target as HTMLElement).closest('.topbar,.preset-strip,.bottom-nav,.frame-handle,.export-bubble,.layers-bubble')) return
  ...
  const onMapTouchMove=(e:React.TouchEvent)=>{
    ...
    if(e.touches.length===1 && g.mode==='pan'){
      ...
      // Permit much smaller map/route compositions.
      setMapZoom(Math.max(.12,Math.min(6,g.zoom*(dist(a,b)/(g.distance||1)))))
  ...
  const moveFrame=(x:number,y:number)=>{
    ...
    setFrameWidth(Math.max(20,Math.min(94,g.w+(x-g.x)/window.innerWidth*100)))
    setFreeHeight(Math.max(14,Math.min(72,g.h+(y-g.y)/window.innerHeight*100)))
  }
  ...
  return <main className="app">
    ...
    {page==='canvas' ? <section className={`map-stage layer-${layer}`}
      onTouchStart={onMapTouchStart}
      onTouchMove={onMapTouchMove}
      onTouchEnd={onMapTouchEnd}
      onTouchCancel={()=>gesture.current=null}>

      {/* A fixed square scene prevents non-uniform stretching of the route. */}
      <div className="scene-square" style={{transform:`translate(calc(-50% + ${pan.x/8}px), calc(-50% + ${pan.y/8}px)) scale(${mapZoom}) rotate(${mapRotation}deg)`}}>
        ...
        <svg className="route-scene" viewBox="0 0 1000 1000">
          <path d={routeD} fill="none" stroke={stroke} strokeWidth={strokeWidth} vectorEffect="non-scaling-stroke" strokeLinecap="round" strokeLinejoin="round"/>
      ...
      <header className="topbar">
        <div/>
        <strong className="title">Route Studio</strong>
      ...
      <div className="preset-row">
        ...
        <button className="export-bubble" onClick={download} aria-label="Export SVG"><Download/></button>
      </div>
      ...
      <div className="history-list">
        {sampleHistory.map(item=><button className="history-card" key={item.id} onClick={()=>setPage('canvas')}>
    ...
    {panel&&<div className="modal-shade" onClick={closePanel}>
      <section className="panel" style={{transform:`translateY(${panelOffset}px)`}} onClick={e=>e.stopPropagation()}>
        ...
        <div className="panel-head">
          <h2>{panel==='route'?'Route':panel==='style'?'Style':panel==='settings'?'Settings':'Map Style'}</h2>
        ...
        {panel==='settings'&&<>
          <div className="settings-section"><h3>Map & Display</h3>
            ...
            <button><span><MapPin/> Units</span><small>Miles · Feet</small></button>
          </div>
          <div className="settings-section"><h3>Editing Defaults</h3>
            <button><span><SlidersHorizontal/> Default Route Style</span><small>Steel Blue</small></button>
            <button><span><RouteIcon/> Default Print Area</span><small>{framePreset}</small></button>
          </div>
